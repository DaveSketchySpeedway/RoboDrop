...
for i = 1:length(data)
    ...
    plot(ot(:,1),v_avg,'g') % transfer function avg
    plot(ch(i).ot,ch(i).v_check,'y') % least square individual
    if i <= nfctm
        plot(ot(:,i),v_ssd(:,i), 'b') % state space individual
    end
    legend('command','measurement','tf avg fit','LS indivi fit', 'ss indivifit')
    title([num2str(order) ' Pump Response Channel ' num2str(i)])
    xlabel('time[s]')
...
legend('exp (commanded)','exp (measured)','sim (pump model)')
title(['Applying Model on Fit Data (Ch' num2str(i) ')'])
...
legend('exp (commanded)','exp (measured)','sim (pump model)')
title(['Applying Model on Test Data (Ch' num2str(i) ')'])
