...
pole_real = 8;
pole_imag = 0;
k = 1 * sqrt(pole_real^2 + pole_imag^2);
sys = tf([-1 2/Ts],[1 2/Ts]) * k * tf(1,[1 pole_real + j*pole_imag]);
...
for c = 1:size(data,2)
    ...
    subplot(2,1,1)
    semilogx(omega, 20*log10(mag_exp(:,c)),'k+')
    hold on
    semilogx(omega, 20*log10(mag_c), 'r')
    semilogx(omega, 20*log10(mag_d), 'b')
    xlabel('frequency[rad/s]')
    ylabel('magnitude[dB]')
    grid minor
    title(['Pump Frequency Response ch' num2str(c)])
        
    subplot(2,1,2)
    semilogx(omega, phase_exp(:,c)./pi()*180,'k+')
    hold on
    semilogx(omega, phase_c, 'r')
